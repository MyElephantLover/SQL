...
select f.name,
    sum(case when w.move not in ('Hadoken', 'Shouoken', 'Kikoken') then f.won else 0 end) as f.won,
    sum(case when w.move not in ('Hadoken', 'Shouoken', 'Kikoken') then f.lost else 0 end) as f.lost 
from fighters f left join winning_moves w on f.move_id = w.id group by f.name order by f.won desc limit 6
;


select f.name, sum(f.won) as won, sum(f.lost) as lost 
from fighters f join winning_moves w on f.move_id = w.id where w.move not in ('Hadoken', 'Shouoken', 'Kikoken')
group by f.name order by won desc limit 6
;
